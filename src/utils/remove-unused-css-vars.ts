 ...
 */
export const removeUnusedCssVars = (css: string, options?: Options) => {
  const root = postcss.parse(css)

  const records = new Map<string, UseRecord>(options?.whiteList?.map(v => [v, { uses: 1, dependencies: new Set(), declarations: new Set() } as UseRecord]))
